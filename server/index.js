...
import { activityTypes, calculateCO2 } from '../shared/emissions.js';
const db = new PrismaClient();
const app = express();
const port = Number(process.env.PORT ?? 3001);
const activityInput = z.object({
    type: z.enum(activityTypes),
    quantity: z.number().finite().positive().max(10000000), date: z.string().date(),
    note: z.string().trim().max(500).optional().default(''), unusual: z.boolean().optional().default(false),
});
app.use(cors());
app.use(express.json({ limit: '32kb' }));
app.get('/api/health', (_req, res) => res.json({ ok: true }));
app.get('/api/activities', async (_req, res, next) => { try {
    res.json(await db.activity.findMany({ orderBy: [{ date: 'desc' }, { createdAt: 'desc' }] }));
}
catch (error) {
    next(error);
} });
...
app.post('/api/activities', async (req, res, next) => {
    try {
        const { type, quantity, date, note, unusual } = activityInput.parse(req.body);
...
app.delete('/api/activities/:id', async (req, res, next) => { try {
...
app.delete('/api/activities', async (_req, res, next) => { try {
...
app.get('/api/settings', async (_req, res, next) => { try {
    res.json(await db.settings.upsert({ where: { id: 1 }, update: {}, create: { id: 1 } }));
}
catch (error) {
    next(error);
} });
app.put('/api/settings', async (req, res, next) => {
    try {
        const { weeklyTarget } = z.object({ weeklyTarget: z.number().finite().positive().max(10000000) }).parse(req.body);
